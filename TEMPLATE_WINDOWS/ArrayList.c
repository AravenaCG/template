...
void al_map (ArrayList* list, void (*func)(void*)/*, int (*criterio)(void*) */)
{
    int i ;
    void * aux ;
    for (i=0;i<al_len(list);i++)
    {
      ...
      /*  if (criterio(aux) ==  1 )
        {
                printf("el item %d cobra mas de 10000\n",i);
*/
            func(aux);
...
/** \brief crea una lista llamando a un puntero a una funcion filtro y pasando solo los elementos que pasaron por el mismo
 *
 * \param listIn ArrayList* Pointer to arrayList
 * \param functionFilter void* es un puntero a una funcion la cual filtra elementos
 * \return listaFiltrada es la lista con los elementos que si pasaron por el filtro
 *
 */
...
ArrayList* al_filter(ArrayList* listIn, int (*functionFilter)(void*))
{
    ...
    if (listIn != NULL)
    {
        for (i=0; i<al_len(listIn); i++)
        {
            ...
            {
                al_add(listaFiltrada,auxEmployee);
            }
